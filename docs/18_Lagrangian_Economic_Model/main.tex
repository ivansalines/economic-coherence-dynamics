...
In this section we outline two conceptual figures that can accompany the theoretical development. They can be implemented either as external PDF graphics or via TikZ, depending on the chosen workflow.
...
% Strong internal links cluster 1
\draw[strong] (A) -- (B);
\draw[strong] (A) -- (C);
\draw[strong] (B) -- (C);
...
% Strong internal links cluster 2
\draw[strong] (D) -- (E);
\draw[strong] (D) -- (F);
\draw[strong] (E) -- (F);
...
% Optional region hints (light braces or labels)
\node[above=4pt of A] {\small Coherence cluster 1};
\node[above=4pt of D] {\small Coherence cluster 2};

\end{tikzpicture}
\caption{
Schematic representation of two coherence clusters in a many-subsystem network.
Thick links indicate strong couplings $J_{ij}$ leading to robust phase alignment within each cluster.
Dashed links represent weaker interactions between clusters, allowing exchange without disrupting internal coherence.}
\label{fig:clusters}
\end{figure}
